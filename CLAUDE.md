...
- Board columns (vendor update v1.1.2, 2026-09-16): tasks carry `column` (board name), `get_project` returns `columns`, and `status` also accepts a column name. Talk about tasks by their column name and re-read the task before relying on it. `aiLocked` columns are hidden and reject AI moves with 403 `aiLock.columnLocked`.
